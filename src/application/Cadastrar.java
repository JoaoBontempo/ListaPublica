...
public class Cadastrar extends Application {
	...
	boolean ValidarCampos() {
		...
		if (!txtUsuario.getText().matches("[a-zA-Z]+"))/* regex para ver somente letras e acentuadas */ {
			Util.MessageBoxShow("Erro ao Cadastrar", "O Usu�rio inserido � inv�lido, insira somente letras",
					AlertType.ERROR);
			return false;
		}

		if (!Validacao.validarEmail(txtEmail.getText())) {
			Util.MessageBoxShow("Erro ao Cadastrar", "O E-mail inserido � inv�lido", AlertType.ERROR);
			return false;
		}
	...
	void TrocarFotoPerfil(ActionEvent event) {
		try {
			...
			if(imagemEscolhida.length() >= 4194304) {
				Util.MessageBoxShow("Inserção de Imagem", "A imagem solicitada excede o tamanho m�ximo de 4Mb",
						AlertType.WARNING);
				return;
			}
			...
			if(!validarImagem(imagemEscolhida)) {
				Util.MessageBoxShow("Imagem corrompida", "A imagem selecionada est� corrompida.");
				return;
			}
				...
				if (txtSenha.getText().equals(txtConfirmarSenha.getText())) {	
					...
					if(caminhoArquivo.length()>0) {
						if(Files.exists(Path.of(caminhoArquivo))) {
							...
							Util.MessageBoxShow("Arquivo inexistente", "O arquivo de imagem de perfil selecionado n�o existe.");
							return;
						}
					}
					...
					Util.MessageBoxShow("Erro ao Cadastrar", "As senhas inseridas n�o correspondem", AlertType.ERROR);
	...
	private boolean validarCredenciais(boolean tipo) throws SQLException {
		...
		if (result.next()) {
			...
			if (num > 1) {
				...
				Util.MessageBoxShow("Erro ao Cadastrar", "As credenciais " + msg + " ja s�o existentes", AlertType.WARNING);
			}
			else
				Util.MessageBoxShow("Erro ao Cadastrar", "A credencial " + msg + " ja � existente", AlertType.WARNING);
	...
	public void start(Stage primaryStage) {
		try {
			...
			primaryStage.setTitle("Lista P�blica - Cadastrar");
